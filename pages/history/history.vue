				...
				<view class="cu-card case">
					<view class="cu-item shadow">
						...
						<view v-show="item.stat == 'RESERVE'" class="cu-form-group">
							<button @click="opCallback(index)" class="cu-btn round sm bg-red">取消</button>
						</view>
						<view v-show="item.stat == 'CHECK_IN'" class="cu-form-group">
							<button @click="opCallback(-1)" class="cu-btn round sm bg-red">结束</button>
						</view>
					</view>
				</view>
			</block>
...
<script>
	...
	export default {
		...
		methods: {
			...
			opHandle(index) {
				var url = index >= 0 ? (cancel_url + this.reservationsList[index].id) : stop_url
